...
if __name__=='__main__':
    ...
    HEA_data = torch.randn([1, 512, 15])
    strain_data = torch.randn([1, 512, 1])
    text_data = torch.randn([1, 1, 1536])
    #start_index传入的是tensor形式还是list形式都可
    #start_index = torch.tensor([5, 45, 46, 8, 9, 48, 52, 12])
    start_index = [5]    
    weights_list, indices_list ,output = model(HEA_data, strain_data, text_data, start_index)
    print(output.shape)           #[1, 512]
    print(weights_list[5].shape, weights_list[5])
    print(indices_list[5].shape, indices_list[5])
