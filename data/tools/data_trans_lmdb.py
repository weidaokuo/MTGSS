...
"""    
data_trans = data_to_lmdb('./lmdb')
# Example data
tuples1 = []
tuples2 = []
for _ in range(1000):  # Assuming you have 1000 tuples
    state = np.random.rand(64, 64, 3)
    next_state = np.random.rand(50, 30, 3)
    reward = np.random.rand()
    action = np.random.rand()
    tuples1.append((state,action, reward, next_state))
    tuples2.append((state, action, reward, next_state))
# Save data to LMDB
data_trans.save_to_lmdb(tuples1)
data_trans.save_to_lmdb(tuples2)   
"""
