...
class Segment_train(object):
    ...
    def __init__(self, args):
        ...
        self.data_loader = DataLoader(lmdb_dataset, batch_size=self.batch_size, shuffle=True, num_workers=0)        
        # Define MSE loss
        self.mse_loss = nn.MSELoss(reduction='mean')

    def train(self, recurrent = True):
        loss_all = [] 
        mse_loss_list = []
        kv_loss_list = []
        diff_loss_list = []
        ...
        for epoch in range(start_epoch, args.pretrain_max_epochs): 
            ...
            for physic_feature_input, strain_input, language_input, stress_output, start_index in self.data_loader:
                ...
                #print("physic_feature_input.shape:", physic_feature_input.shape)
                #print("strain_input.shape:", strain_input.shape)
                #print("language_input.shape:", language_input.shape)
                #print("start_index:", start_index)
                outputs = self.model(physic_feature_input, strain_input, language_input, start_index) 
                #print("outputs.shape:", outputs.shape, "stress_output.shape:", stress_output.shape)
            ...
            if epoch % 100 == 0:                   
                ...
                save_executor.submit(torch.save, checkpoint, f"./weights/checkpoint_pretrain.pth")
                
                save_executor.submit(np.savetxt, f"./loss/{train_order}train_loss_all.txt", np.array(loss_all), delimiter=" ")
                
                save_executor.submit(np.savetxt, f"./loss/{train_order}train_loss_mse.txt", np.array(mse_loss_list), delimiter=" ")                

                save_executor.submit(np.savetxt, f"./loss/{train_order}train_loss_kv.txt", np.array(kv_loss_list), delimiter=" ")

                save_executor.submit(np.savetxt, f"./loss/{train_order}train_loss_diff.txt", np.array(diff_loss_list), delimiter=" ") 
                
                #save_executor.submit(np.savetxt, f"./loss/{train_order}learning_rate.txt", np.array(learning_rate), delimiter=" ")
               
            print("0Epoch_all:{} loss:".format(epoch), sum(loss_list) / len(loss_list)) 
            #print("1Epoch_loss_mse:{} loss:".format(epoch), sum(loss_mse) / len(loss_mse)) 
            #print("2Epoch_loss_kv:{} loss:".format(epoch), sum(loss_kv) / len(loss_kv)) 
            #print("3Epoch_all_diff:{} loss:".format(epoch), sum(loss_diff) / len(loss_diff)) 
    ...
    def warmup_lr_lambda(self, current_step: int):
        # The learning rate uses a warmup strategy
